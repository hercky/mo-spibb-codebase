...
from gridworld.lag_baseline.lag_agent import Lagrangian
...
def benchmark_delta_agents(env_name,
                           num_runs=5,
    ...
    for run in tqdm(range(num_runs)):
        ...
        logger.log(f"Found env for run {run} in {i} iterations")
            ...
            for nb_traj in tqdm(nb_trajectories_list, desc="Nested Loop (nb_traj)"):
                ...
                for agent in agent_list:
                    ...
                    for coeff in agent.coeff_list or [None]:
                        ...
                        if "SPIBB" in agent.__class__.__name__:
                        ...
                        elif "Lagrangian" in agent.__class__.__name__:
                            # =========================================================================#
                            # Lagrangian agent (Le et al.)
                            # =========================================================================#

                            piSolution_R_true_performance, piSolution_C_true_performance, true_failure_rate = agent.make_policy_iteration_operator(
                                P=P_hat,
                                R=R_star,
                                C=C_star,
                                discount=discount,
                            ...
                            # w.r.t. P_hat
                            # skip because we don't have pi_solution in this case
                            ...
                            results.append([run,  # exp run
                                            ...
                                            "_".join(map(str, coeff)),  # conver coeff to string
                                            None, None,  # spibb specific params
                                            -1.0 , agent.lr_eta, agent.lambda_bound,
                                            # hcpi specific params overrriden
                                            # (delta, IS, CI) -> (-1, lr_eta, lambda_bound)
                                            ])
...
if __name__ == '__main__':
    ...
    lambda_R_vals = [1.0]  # >=0
    lambda_C_vals = [1.0]  # >=0
    # lambda_R_vals = [0.0, 0.1, 1.0, 10.0]  # >=0
    # lambda_C_vals = [0.0, 0.1, 1.0, 10.0]  # >=0
    ...
    if args.agent == 's-opt':
        ...
        agent_kwargs = dict(lower_bound_strategy="student_t_test",
                            # lower_bound_strategy="CI",
                            estimator_type=args.ope,
                            # estimator_type="doubly_robust",
                            coeff_list=lambda_coeffs,
                            training_size=0.7,
                            )
        ...
        agent_kwargs = dict(coeff_list=lambda_coeffs,
                            n_iter=250,
                            norm_bound=10,
                            lr_eta=args.eps,
                            dual_gap=0.1)

        agent = Lagrangian(**agent_kwargs)
    else:
        raise Exception("not implemented yet")

    benchmark_delta_agents(args.env_name,
