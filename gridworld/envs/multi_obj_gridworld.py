"""
Environment for the scaling experiments in Appendix D

Check the safety_gridworld.py for more details
"""
...
def generate_maze(size=27, num_constraints=1, obstacle_density=0.3, rand_goal=False):
    """
    ...
    maze_tensor = np.zeros((len(COLOR) + num_constraints - 1, size, size))
    ...
    # put the pits
    for pit_type in range(num_constraints):

        # create the the pits for this constraint here
        for i in range(0, mx):
            for j in range(0, my):
                ...
                if np.random.rand() < obstacle_density:
                    ...
                    maze_tensor[PIT + pit_type][j+1][i+1] = 1.0
...
class CostlyPitWorld(gym.Env):
    ...
    def cord_to_index(self, y, x):
        ...
        #     raise Exception("trying to access the unreachable block")
    ...
    def compute_cmdp_matrices(self):
        ...
        for constraint_type in range(self.num_constraints):

            # create a matrix for this constraint
            ...
            for y in range(1, my - 1):
                for x in range(1, mx - 1):
                    ...
                    if self.maze[PIT + constraint_type][y][x] == 1.0:
                        C[current_state, :] = self.constraint_cost

            # append the C to the list
            C_list.append(C)
    ...
    def step(self, action):
        ...
        constraint = np.zeros(self.num_constraints)
        info = {}
        ...
        # if reached PIT
        for pit_type in range(self.num_constraints):
            if self.maze[PIT + pit_type][self.agent_pos[0]][self.agent_pos[1]] == 1.0:
                constraint[pit_type] = self.constraint_cost
...
if __name__ == "__main__":
    ...
    env = CostlyPitWorld(num_constraints=4, size=5+2, feature_type="tabular", rand_goal=False, rand_transition=True, random_action_prob=0.0)
